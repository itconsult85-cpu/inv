...
Forecast kebutuhan material berdasarkan outstanding dan stok produk
...
<style>
    ...
    .forecast-card.total { border-color: #17a2b8; }
    .forecast-card.aman { border-color: #28a745; }
    .forecast-card.kurang { border-color: #dc3545; }
    ...
    #tabelKebutuhan th, #tabelKebutuhan td { vertical-align: middle; }
    .angka { text-align: right; white-space: nowrap; }
    .status-badge { min-width: 105px; display: inline-block; }
...
<div class="card">
    <div class="card-header">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mt-2">
            ...
            <div>
                ...
                <button type="button" id="btnCetak" class="btn btn-info mr-1">
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-6 col-lg-3 mb-3">
                <div class="card forecast-card total mb-0">
                    <div class="card-body">
                        <div class="value" id="ringkasanTotal">0</div>
                        <div class="label">Material Dibutuhkan</div>
                    </div>
                </div>
            </div>
            <div class="col-6 col-lg-3 mb-3">
                <div class="card forecast-card kurang mb-0">
                    <div class="card-body">
                        <div class="value text-danger" id="ringkasanKurang">0</div>
                        <div class="label">Stok Kurang</div>
                    </div>
                </div>
            </div>
            <div class="col-6 col-lg-3 mb-3">
        ...
        <div class="table-responsive">
            <table id="tabelKebutuhan" class="table table-bordered table-striped table-hover">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Kode Material</th>
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="judulDetail">Detail Kebutuhan Material</h5>
...
<script>
    ...
    function badgeStatus(status) {
        if (status === 'kurang') {
            return '<span class="badge badge-danger status-badge">Kurang</span>';
        }
        if (status === 'belum_lengkap') {
            return '<span class="badge badge-warning status-badge">Belum Lengkap</span>';
        }
        return '<span class="badge badge-success status-badge">Aman</span>';
    }

    function tampilkanRingkasan(summary) {
        $('#ringkasanTotal').text(summary.total || 0);
        $('#ringkasanKurang').text(summary.kurang || 0);
    ...
    function renderTabel(rows) {
        if ($.fn.DataTable.isDataTable('#tabelKebutuhan')) {
            $('#tabelKebutuhan').DataTable().destroy();
        }

        const body = $('#tabelKebutuhan tbody').empty();
        rows.forEach((row, index) => {
            ...
            body.append(`
                <tr>
                    ...
                    <td class="angka ${Number(row.kekurangan) > 0 ? 'text-danger font-weight-bold' : ''}">${formatAngka(row.kekurangan)}</td>
                    <td class="angka ${Number(row.saran_beli) > 0 ? 'text-primary font-weight-bold' : ''}">${formatAngka(row.saran_beli)}</td>
                    <td class="text-center">${badgeStatus(row.status)}</td>
        ...
        tabelKebutuhan = $('#tabelKebutuhan').DataTable({
            responsive: true,
            autoWidth: false,
            stateSave: true,
            stateDuration: -1,
            ...
            order: [[11, 'asc'], [1, 'asc']],
            language: {
                emptyTable: 'Tidak ada kebutuhan material untuk filter yang dipilih.'
            },
            columnDefs: [
    ...
    function muatData() {
        ...
        $.ajax({
            url: '<?= site_url('kebutuhanmaterial/data') ?>',
            method: 'GET',
            data: parameterFilter(),
            dataType: 'json'
        }).done(response => {
            dataKebutuhan = response.data || [];
    ...
    $(document).on('click', '.btn-detail', function() {
        const row = dataKebutuhan[Number($(this).data('index'))];
        if (!row) return;

        $('#judulDetail').text('Detail ' + row.kode_material + ' - ' + row.nama_material);
        ...
        (row.details || []).forEach(detail => {
            body.append(`
                <tr>
                    ...
                    <td class="angka">${formatAngka(detail.outstanding)} Pcs</td>
                    <td class="angka">${formatAngka(detail.stok_produk)} Pcs</td>
    ...
    $('#btnReset').on('click', function() {
        $('#filterPelanggan, #filterProduk, #filterMaterial, #filterStatus').val('');
        $('#filterPelangganInput, #filterProdukInput, #filterMaterialInput').val('');
        muatData();
    });
    $('#btnCetak').on('click', function() {
        const query = $.param(parameterFilter());
        window.open('<?= site_url('kebutuhanmaterial/cetak') ?>?' + query, '_blank');
    });
