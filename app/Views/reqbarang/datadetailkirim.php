<?php if (\App\Libraries\AccessControl::can('produk.transfer.override_stok')) :  ?><div class="row mb-3">
        <div class="col-md-4">
    ...
    <tbody>
        ...
        ?>
            <tr>
                ...
                <td style="text-align: center;"><?= $row['detkodebrg'] ?>
                ...
                <td style="text-align: right;"><?= number_format($row['detqty'], 0, ",", ".") ?> Pcs
                    ...
                    <input type="hidden" value="<?= $row['detqty'] ?>" id="detjml">
                    <?php if (\App\Libraries\AccessControl::can('produk.transfer.override_stok')) :  ?>
                        <input type="text" value="<?= $row['stok'] ?>" id="stok">
                        <input type="text" value="<?= $row['detqty'] ?>" id="detjml">
                    <?php endif; ?>
                </td>
                ...
                <td style="text-align: right;"><?= number_format($row['detsubtotal'], 4, ",", ".") ?> Kg
                    <input type="hidden" value="<?= $row['detqty'] + $row['detkurang'] ?>" id="detqty">
                    <?php if (\App\Libraries\AccessControl::can('produk.transfer.override_stok')) :  ?>
                        <input type="text" value="<?= $row['detqty'] + $row['detkurang'] ?>" id="detqty">
                    <?php endif; ?>
                </td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        //         var kurang = parseInt($(this).find('#detqty').val()) || 0;
        ...
        $('#datadetail tbody').on('click', 'tr', function() {
            ...
            $('#kodebarang').val(row.find('td:eq(1)').text().trim());
            $('#idbarang').val(row.find('td:eq(1) input').val());
            $('#namabarang').val(row.find('td:eq(2)').text().trim());
            $('#berat').val(row.find('td:eq(3) input').val());
            $('#stok').val(row.find('td:eq(4) input').val());
            $('#detkurang').val(row.find('td:eq(5) input').val());
            $('#idgudang').val(row.find('td:eq(7) input').val());
            $('#jml').val(row.find('#detjml').val());
            $('#detqty').val(row.find('#detqty').val());
            $('#tombolBatal').fadeIn();
            $('#tombolEditItem').fadeIn();
            $('#kodebarang').prop('readonly', true);
            $('#jenisItemProses').prop('disabled', true);
