...
<?php if ($source): ?>
    ...
    <div class="row">
        <div class="col-md-3 form-group"><label>No. Invoice Supplier</label><input name="invoice_no" class="form-control" value="<?= old('invoice_no') ?>" required></div>
        <div class="col-md-3 form-group"><label>Tanggal Invoice</label><input type="date" name="invoice_date" class="form-control" value="<?= old('invoice_date', date('Y-m-d')) ?>" required></div>
        <div class="col-md-3 form-group"><label>Supplier</label><input class="form-control" value="<?= esc($source['header']['supplier_name']) ?>" readonly></div>
        <div class="col-md-3 form-group"><label><?= $selectedType === 'po_keluar' ? 'No. PO Keluar' : 'No. Transaksi Masuk' ?></label><input class="form-control" value="<?= esc($source['header']['source_label'] ?? $source['header']['source_no']) ?>" readonly></div>
    </div>
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-4 form-group">
                    ...
                    <div class="input-group">
                        <div class="input-group-prepend">
                            <div class="input-group-text"><input type="checkbox" name="ppn_enabled" id="ppnEnabled" value="1" <?= old('ppn_enabled', '1') ? 'checked' : '' ?>></div>
                        </div>
                ...
                <div class="col-md-4 form-group">
                    ...
                    <div class="input-group">
                        <div class="input-group-prepend">
                            <div class="input-group-text"><input type="checkbox" name="pph_enabled" id="pphEnabled" value="1" <?= old('pph_enabled', '1') ? 'checked' : '' ?>></div>
                        </div>
                ...
                <div class="col-md-4 form-group mb-0">
                    ...
                    <div class="d-flex align-items-center mb-2">
                        <div class="input-group mr-2" style="max-width: 8rem;">
                            <div class="input-group-prepend">
                                <div class="input-group-text"><input type="checkbox" name="dp_enabled" id="dpEnabled" value="1" <?= old('dp_enabled') ? 'checked' : '' ?>></div>
                            </div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered" id="detailIn">
            <thead>
                <tr>
                    <th>No</th>
                    <th>No. Surat Jalan</th>
            ...
            <tbody><?php foreach ($source['lines'] as $i => $line): ?><tr>
                        <td><?= $i + 1 ?></td>
                        <td><?= esc($line['no_surat_jalan'] ?: '-') ?></td>
                        <td><?= esc($line['item_code']) ?></td>
                        <td><?= esc($line['item_name']) ?></td>
                        <td class="qty text-right" data-value="<?= (float)$line['qty'] ?>"><?= number_format($line['qty'], 0, ',', '.') ?></td>
                        <td><?= esc($line['unit']) ?></td>
                        <td><input type="number" min="0" step="0.01" name="harga[<?= esc($line['source_detail_id']) ?>]" class="form-control harga text-right" value="<?= old('harga.' . $line['source_detail_id'], $line['unit_price_default'] ?? 0) ?>" required></td>
                        <td class="amount text-right">Rp 0</td>
                    </tr><?php endforeach ?></tbody>
            <tfoot>
                <tr>
                    <th colspan="7" class="text-right">Subtotal</th>
                    <th id="subtotal" class="text-right">Rp 0</th>
                </tr>
                <tr id="rowPpn">
                    <th colspan="7" class="text-right">PPN <span id="ppnLabel">11%</span></th>
                    <th id="ppn" class="text-right">Rp 0</th>
                </tr>
                <tr id="rowPph">
                    <th colspan="7" class="text-right">PPh 23 (<span id="pphLabel">2%</span>)</th>
                    <th id="pph" class="text-right">Rp 0</th>
                </tr>
                <tr id="rowDp">
                    <th colspan="7" class="text-right">DP <span id="dpLabel">50%</span></th>
                    <th id="dp" class="text-right">Rp 0</th>
                </tr>
                <tr>
                    <th colspan="7" class="text-right">Grand Total</th>
                    <th id="grand" class="text-right">Rp 0</th>
                </tr>
            </tfoot>
        </table>
    </div><button class="btn btn-success"><i class="fas fa-save"></i> Simpan Invoice In</button><?= form_close() ?>
...
<script>
    ...
    function hitung() {
        let sub = 0;
        $('#detailIn tbody tr').each(function() {
            const q = parseFloat($(this).find('.qty').data('value')) || 0;
            const h = parseFloat($(this).find('.harga').val()) || 0;
            const a = q * h;
            sub += a;
            $(this).find('.amount').text('Rp ' + Math.round(a).toLocaleString('id-ID'));
        });
        ...
        const dpAmount = dpEnabled ? (dpMode === 'amount' ? Math.min(dpValue, sub + ppn) : (sub) * (dpPercent / 100)) : 0;
        const grand = Math.max((sub + ppn) - pph - dpAmount, 0);

        $('#subtotal').text('Rp ' + Math.round(sub).toLocaleString('id-ID'));
        $('#ppnLabel').text((ppnPercent || 0).toLocaleString('id-ID', {
            maximumFractionDigits: 2
        }) + '%');
        $('#ppn').text('Rp ' + Math.round(ppn).toLocaleString('id-ID'));
        $('#rowPpn').toggle(ppnEnabled);
        $('#pphLabel').text((pphPercent || 0).toLocaleString('id-ID', {
            maximumFractionDigits: 2
        }) + '%');
        $('#pph').text('Rp ' + Math.round(pph).toLocaleString('id-ID'));
        $('#rowPph').toggle(pphEnabled);
        $('#dpLabel').text(dpMode === 'amount' ? 'Nominal DP' : 'DP ' + (dpPercent || 0).toLocaleString('id-ID', {
            maximumFractionDigits: 2
        }) + '%');
