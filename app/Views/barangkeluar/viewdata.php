...
<div class="tab-content border border-top-0 p-3" id="tabPengirimanUtamaContent">
    <div class="tab-pane fade show active" id="tab-daftar" role="tabpanel">
        <div class="pgr-toolbar">
            <div class="pgr-filter-anchor">
                <div class="pgr-search-shell">
                    ...
                    <input type="search" id="daftarSearchInput" class="pgr-search-input" placeholder="Search anything..." aria-label="Search anything">
                    <button type="button" class="pgr-filter-toggle" id="daftarFilterToggle" title="Buka filter" aria-controls="daftarFilterPanel" aria-expanded="false">
                ...
                <section class="pgr-filter-panel" id="daftarFilterPanel" aria-hidden="true">
                    ...
                    <div class="pgr-filter-section">
                        ...
                        <div class="pgr-filter-field-row">
                            <input type="date" name="tglawal" id="daftarTglawal" class="pgr-filter-date" aria-label="Start date">
                            <input type="date" name="tglakhir" id="daftarTglakhir" class="pgr-filter-date" aria-label="End date">
                        </div>
                    </div>
        ...
        <table id="dataPengirimanGabungan" class="table table-bordered table-striped dataTable dtr-inline collapsed" style="width:100%">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Tanggal</th>
    ...
    <div class="tab-pane fade" id="tab-permintaan" role="tabpanel">
        <div class="pgr-toolbar">
            <div class="pgr-filter-anchor">
                <div class="pgr-search-shell">
                    <i class="fas fa-search"></i>
                    <input type="search" id="permintaanSearchInput" class="pgr-search-input" placeholder="Search anything..." aria-label="Search anything">
                    <button type="button" class="pgr-filter-toggle" id="permintaanFilterToggle" title="Buka filter" aria-controls="permintaanFilterPanel" aria-expanded="false">
                        <i class="fas fa-sliders-h"></i>
                    </button>
                </div>
        ...
        <table id="dataPermintaanPengiriman" class="table table-bordered table-striped dataTable dtr-inline collapsed" style="width:100%">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Tanggal</th>
    ...
    <div class="tab-pane fade" id="tab-riwayat" role="tabpanel">
        <div class="pgr-toolbar">
            <div class="pgr-filter-anchor">
                <div class="pgr-search-shell">
                    <i class="fas fa-search"></i>
                    <input type="search" id="riwayatSearchInput" class="pgr-search-input" placeholder="Search anything..." aria-label="Search anything">
                    <button type="button" class="pgr-filter-toggle" id="riwayatFilterToggle" title="Buka filter" aria-controls="riwayatFilterPanel" aria-expanded="false">
                        <i class="fas fa-sliders-h"></i>
                    </button>
                </div>
                ...
                <section class="pgr-filter-panel" id="riwayatFilterPanel" aria-hidden="true">
                    ...
                    <div class="pgr-filter-section">
                        <div class="pgr-filter-label">Custom Date Range</div>
                        <div class="pgr-filter-field-row">
                            <input type="date" name="tglawal" id="tglawal" class="pgr-filter-date" aria-label="Start date">
                            <input type="date" name="tglakhir" id="tglakhir" class="pgr-filter-date" aria-label="End date">
                        </div>
                    </div>
    ...
    function editDokumenPengiriman(noDoHash) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const labelItemDasar = (item) => `${item.no_po} - ${item.kode_produk} - Qty ${formatQty(item.qty)}`;
                const labelItemText = (item) => labelItemDasar(item) + (item.no_btb ? ` (${item.no_btb})` : '');
                const labelItem = (item) => {
                    const dasar = `<span style="font-weight:normal;">${escapeHtmlPengiriman(labelItemDasar(item))}</span>`;
                    return item.no_btb ? `${dasar} <strong>(${escapeHtmlPengiriman(item.no_btb)})</strong>` : dasar;
                };
                ...
                showBootstrapModal({
                    ...
                    didOpen: () => {
                        ...
                        const tampilkanView = function() {
                            ...
                            if (ids.length === 0) {
                                $('#btbViewNoBtb').text('-');
                                $('#btbViewFile').html('<span class="text-muted">Pilih minimal 1 item.</span>');
                            ...
                            const semuaSama = dipilih.every(item => item.no_btb === dipilih[0].no_btb && item.btb_file_url === dipilih[0].btb_file_url);

                            if (!semuaSama) {
                                $('#btbViewNoBtb').text('-');
                                $('#btbViewFile').html('<span class="text-muted">Item-item terpilih punya No BTB / file berbeda-beda. Isi baru untuk menerapkan ke semua item terpilih.</span>');
                            ...
                            $('#btnHapusBtb').prop('disabled', !item.no_btb && !item.btb_file_url);
                        };
                        ...
                        if (adaBanyakItem) {
                            ...
                            $('.btb-item-checkbox').on('change', function() {
                                ...
                                $('#btbPilihSemua').prop('checked', totalChecked === daftarItem.length);
                                tampilkanView();
                            });
                        }
                        ...
                        $('#btnEditBtb').on('click', function() {
                            const ids = idTerpilih();
                            const dipilih = daftarItem.filter(item => ids.includes(item.id));
                            const semuaSama = dipilih.length > 0 && dipilih.every(item => item.no_btb === dipilih[0].no_btb);
                        ...
                        $('#btnSimpanBtb').on('click', function() {
                            ...
                            formData.append('no_btb', document.getElementById('noBtbPengiriman').value.trim());
            ...
            success: function(response) {
                ...
                showBootstrapModal({
                    ...
                    if (result.isConfirmed) {
                        ...
                        let windowCetak = window.open('<?= site_url('barangkeluar/cetak-do') ?>/' + encodeURIComponent(faktur) + '?' + params.toString(),
                            "Cetak DO",
