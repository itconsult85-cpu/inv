...
class Dashboard extends BaseController
{
    ...
    {
        if (!\App\Libraries\AccessControl::can('dashboard.view')) {
    ...
    public function getChartDataPo5()
    {
        ...
        foreach (range(1, 12) as $bulan) {
            ...
            foreach ($currentTopProducts as $product) {
                $kode = $product['detkodebrg'];
                if (!isset($datasetsCurrentYear[$kode])) {
                    $datasetsCurrentYear[$kode] = [
                        'label' => $kode . ' ' . $yearpo,
                        'backgroundColor' => '#36A2EB',
                        'borderColor' => '#36A2EB',
                        'borderWidth' => 1,
                ...
                $datasetsCurrentYear[$kode]['data'][$bulan - 1] = $product['total'];
            }

            foreach ($previousTopProducts as $product) {
                $kode = $product['detkodebrg'];
                if (!isset($datasetsPreviousYear[$kode])) {
                    $datasetsPreviousYear[$kode] = [
                        'label' => $kode . ' ' . ($yearpo - 1),
                        'backgroundColor' => '#FF0000',
                        'borderColor' => '#FF0000',
                        'borderWidth' => 1,
                ...
                $datasetsPreviousYear[$kode]['data'][$bulan - 1] = $product['total'];
            }
        }

        $finalDatasets = array_merge(array_values($datasetsCurrentYear), array_values($datasetsPreviousYear));
